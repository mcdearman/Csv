//! Finds the `csv` and `csv-core` sources that Cargo fetched, for `main.rs` to
//! fingerprint: `src/` ports them by hand.
...
fn main() {
    ...
    let dir = upstream_dir("csv");
    println!("cargo:rustc-env=UPSTREAM_DIR={}", dir.display());
    let core = upstream_dir("csv-core");
    let files = [
        "src/lib.rs",
        "src/reader.rs",
        "src/writer.rs",
    ...
    .map(|f| dir.join(f))
    .into_iter()
    .chain(["src/lib.rs", "src/reader.rs", "src/writer.rs"].map(|f| core.join(f)));
    for path in files {
        sources.push_str(
